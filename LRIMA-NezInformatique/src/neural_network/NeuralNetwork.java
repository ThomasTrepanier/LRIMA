...
public class NeuralNetwork {
    ...
    public static void main(String[] args) {
    	...
    	layers[1] = new Layer(784,6); // Hidden Layer 2,6
    	...
    	while(sucessRate < 0.6f) {
//          System.out.println("============");
//          System.out.println("Output before training");
//          System.out.println("============");
//          for(int i = 0; i < testSet.length * percent / 2; i++) {
//              forward(testSet[i].data);
//              System.out.println(layers[2].neurons[0].value * 9 + " expected: " + testSet[i].expectedOutput[0] * 9);
//          }
         
          train(1, 0.05f, percent);
          ...
          for(int i = 0; i < testSet.length * percent / 2; i++) {
              forward(testSet[i].data);
              System.out.println(layers[2].neurons[0].value + " expected: " + testSet[i].expectedOutput[0]);
          }
          
//          int testAmout = 1000;
//          CreateTestData(testAmout);
          sucessRate = calculateSucessRate() * 100;
    ...
    public static void train(int training_iterations,float learning_rate, float percentageOfDataSetToUse) {
    ...
    private static float calculateSucessRate() {
    	int sucess = 0;
    	int fail = 0;
		for(int i = 0 ; i < testSet.length; i++) {
			forward(testSet[i].data);
			for(int j = 0; j < layers[layers.length - 1].neurons.length; j++) { //For all neurons in final layer
				float value = layers[layers.length - 1].neurons[j].value;
				float expectedValue = testSet[i].expectedOutput[j];
				if(Math.abs(expectedValue - value) < 0.1)
					sucess++;
				else
					fail++;
			}
